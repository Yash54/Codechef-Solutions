...
void findMinimum(V<ll> swapElements, P<ll, bool> minimumCost)
{
    sort(all(swapElements));
    ll cnt = 0;
    loop(i, 0, swapElements.size() / 2)
    {
        if (minimumCost.F * 2 < swapElements[i])
        {
            cnt += (minimumCost.F * 2);
        }
        else
...
void countMinimumCost(UM<ll, ll> freqA, UM<ll, ll> freqB, ST<ll> uniqueSort)
{
    V<ll> swapElements;
    bool flag = false;

    for (auto i : uniqueSort)
    {
        if (abs(freqA[i] - freqB[i]) % 2 == 0)
        {
            if (freqA[i] == freqB[i])
            {
                continue;
            ...
            {
                loop(j, 0, abs(freqA[i] - freqB[i]) / 2)
                {
                    swapElements.pb(i);
                }
            }
    ...
    ll mi = *uniqueSort.begin();
    P<ll, bool> minimumCost = {mi, flag};
    findMinimum(swapElements, minimumCost);
}

void takeInput(UM<ll, ll> &freqA, UM<ll, ll> &freqB, ST<ll> &uniqueSort)
{
    ll n;
    cin >> n;
    ll a[n], b[n];
...
int main(int argc, const char *argv[])
{
    ...
    while (t--)
    {
        UM<ll, ll> freqA, freqB;
        ST<ll> uniqueSort;
        takeInput(freqA, freqB, uniqueSort);
        countMinimumCost(freqA, freqB, uniqueSort);
    }
    return 0;
}