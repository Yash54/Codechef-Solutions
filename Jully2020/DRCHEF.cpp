...
int main(int argc, char const *argv[])
{
    ...
    while (t--)
    {
        ...
        ll highPopulation = upper_bound(population, population + n, cures) - population;
        ll smallerCountries = 0;
        ll temp = cures;

        loopr(i,highPopulation-1,0)
        {
            if (2 * population[i] < temp)
            {
                smallerCountries = i + 1;
                break;
            }
            else
            {
        ...
        days += (highPopulation - smallerCountries);
        
        if (days > 0)
        {
            cures = population[highPopulation - 1] * 2;
        }
        
        days = highPopulation;

        ll nextCountry = upper_bound(population, population + n, cures) - population;
        ...
        while(1)
        {
            ...
            days += (nextCountry - highPopulation);

            if (nextCountry == highPopulation)
            {

                if (nextCountry == n - 1)
                {

                    temp = population[nextCountry];
                    population[nextCountry] -= cures;

                    population[nextCountry] = min(2 * population[nextCountry], temp);
                }
                days++;
                cures *= 2;
            }
            ...
            {
                cures = population[nextCountry - 1] * 2;
            }

            highPopulation = nextCountry;
            nextCountry = upper_bound(population, population + n, cures) - population;
        }
        cout << days << "\n";
    }
    return 0;
